...
const redirect_uri = 'https://jammming-by-shuyang.surge.sh'; // Your redirect uri ex http://localhost:3000/ https://jammming-by-shuyang.surge.sh
...
const Spotify = {
    ...
    //             } else {
    //                 localStorage.setItem('accessToken', JSON.stringify(url.match(/access_token=([^&]*)/)[1]))
    ...
    async getAccessToken() { 
        try {
            if(access_token) {
                ...
                const url = window.location.href;
                const fromLocalStorage = JSON.parse(localStorage.getItem('accessToken'))
                if(fromLocalStorage){
